...
(() => {
  ...
  const agentLogos={hermes:ICONS.hermes[1],codex:ICONS.codex[1],claude:ICONS.claude[1],openclaw:ICONS.openclaw[1]};
  ...
  function renderOpayaNav(){
    const o=opaya(),label=o.busy?'Working...':o.configured?(o.config.model||'Ready'):'Set up';
  ...
  function renderOpaya(){
    ...
    $('#compose-hint').textContent=o.busy?(o.status||'Working'):o.configured?`${o.presets?.[o.config.preset]?.label||'Model'} / ${o.config.model}`:'Connect a model to start';
  }
  ...
  function openOpayaConfig(){
    const o=opaya(),presets=o.presets||{},c=o.config||{},current=c.preset||'anthropic';
    modal('Opaya Agent model','Works like a Hermes gateway connection: an OpenAI-compatible API, a model ID and, if needed, an API key.',`<form id="opaya-config-form"><div class="preset-grid">${Object.entries(presets).map(([id,p])=>`<label class="preset-option"><input type="radio" name="preset" value="${esc(id)}" ${id===current?'checked':''}><span><strong>${esc(p.label)}</strong><small>${esc(p.baseUrl||'Any /v1 endpoint')}</small></span></label>`).join('')}</div><div class="form-grid">${inputField('baseUrl','API base URL',c.baseUrl||presets[current]?.baseUrl||'','https://.../v1','required')}${inputField('model','Model ID',c.model||presets[current]?.model||'','e.g. claude-sonnet-5, gpt-..., llama3.1','required list="opaya-models" autocomplete="off"')}</div><datalist id="opaya-models"></datalist><label class="field"><span>API key ${o.hasKey?'<em>stored securely</em>':''}</span><input name="apiKey" type="password" autocomplete="new-password" placeholder="${o.hasKey?'Leave empty to keep the saved key':'Not needed for Ollama or LM Studio'}"></label><label class="check-row"><input type="checkbox" name="remember" ${state.secureStorage?'checked':''}> Remember key with OS encryption <small>${state.secureStorage?'Protected by your OS keychain':'Unavailable here: the key stays in memory only'}</small></label><div class="opaya-boundary"><strong>Safety boundary</strong><p>The Opaya Agent works only through Opaya's own tools: connections, machines, discovery, fixed diagnostics and vendor install commands. Each change or command opens an approval dialog. It never receives your API tokens and cannot edit the app. Its chat and notes live in <code>${esc(o.home||'opaya-agent')}</code>.</p></div><p id="opaya-test-result" class="field-help"></p><div class="modal-footer"><div>${o.hasKey?'<button type="button" class="danger-text" data-action="opaya-forget-key">Forget saved key</button>':''}</div><div><button type="button" class="secondary" id="opaya-test">Test connection</button><button type="submit" class="primary">Save</button></div></div></form>`,true);
    const form=$('#opaya-config-form');
    form.addEventListener('change',event=>{if(event.target.name!=='preset')return;const p=presets[event.target.value]||{};form.elements.baseUrl.value=p.baseUrl||'';form.elements.model.value=p.model||'';});
    ...
    $('#opaya-test').onclick=()=>action(async()=>{const out=$('#opaya-test-result');out.textContent='Testing...';try{const v=values();if(!v.model)v.model='test';await api.opayaSaveConfig(v);const r=await api.opayaTest();out.textContent=r.message;$('#opaya-models').innerHTML=r.models.map(m=>`<option value="${esc(m)}">`).join('');}catch(e){out.textContent=e.message;}});
  ...
  function openInstall(hostId=''){
    const list=state.frameworks||[],host=state.hosts.find(h=>h.id===hostId);
    modal('Install an agent.',host?`Installs on ${host.name} over SSH, in a visible terminal.`:'Installs on this computer, in a visible terminal. You see the exact command first.',`<div class="install-target"><span>Install on</span><button class="target-chip ${host?'':'selected'}" data-action="install-target" data-id="">This computer</button>${state.hosts.map(h=>`<button class="target-chip ${h.id===hostId?'selected':''}" data-action="install-target" data-id="${esc(h.id)}">${esc(h.name)}</button>`).join('')}<button class="target-chip add" data-action="hosts">+ Machine</button></div><div class="install-grid">${list.map(f=>{const ok=host?f.remote:f.local;return `<article class="install-card ${ok?'':'unavailable'}"><div class="install-card-top"><span class="agent-avatar large ${esc(f.provider)} ${f.icon?'':'no-logo'}">${f.icon&&ICONS[f.icon]?iconImg(ICONS[f.icon][1]):'<span class="mark custom-mark"><i></i></span>'}</span><div><h3>${esc(f.name)}</h3>${f.requires?`<small>Requires ${esc(f.requires)}</small>`:f.runtime?'<small>Model runtime</small>':''}</div></div><p>${esc(f.description)}</p><button class="${ok?'secondary':'subtle'}" data-action="install-framework" data-id="${esc(f.id)}" data-host="${esc(hostId)}" ${ok?'':'disabled title="No native installer for this system. Install it on a Linux or macOS machine or WSL."'}>${ok?'Install':'Not on this system'}</button></article>`;}).join('')}</div><div class="modal-note"><span class="status-dot connected"></span> After an install, run Discover to add the agent, or ask the Opaya Agent to do it for you.</div>`,true);
  }
