...
(() => {
  ...
  function applyState(next) {
    ...
    if(!initialized){overview=state.view?.overview??!state.activeAgentId;applyTheme(state.view?.theme||'dark');for(const [key,value]of Object.entries(state.drafts||{}))drafts.set(key,value);initialized=true;if(state.recoveryNotice)toast(state.recoveryNotice,true);$('.search-trigger kbd').textContent=`${mod()}K`;}
    render();
  }
  function render() {
  ...
  async function selectAgent(id){overview=false;closeModal();await api.select({id});render();saveView();}
  function agentMenu(a){
    ...
    return [
      ...
      '-',
      {icon:'&#10005;',label:'Remove connection...',danger:true,run:async()=>{if(await api.removeAgent({id})){closeModal();if(state.activeAgentId===id)overview=true;await refresh();toast('Connection removed.');}}}
    ];
  }
