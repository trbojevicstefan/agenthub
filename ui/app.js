...
(() => {
  ...
  const initials = {hermes:'H',codex:'>_',claude:'C',openclaw:'O',custom:'A'};
  let state = {agents:[],hosts:[],conversations:[],histories:{},secureStorage:false}, overview = true, renderKey = '', initialized = false;
  ...
  const saveView = () => { if(api.saveView)save(api.saveView({overview,terminalVisible:!$('#terminal-panel').hidden,terminalId:currentTerminal})); };
  ...
  const badge = (a, large=false) => `<span class="agent-avatar ${esc(a.provider)} ${large?'large':''}" aria-hidden="true">${esc(initials[a.provider]||'A')}</span>`;
  ...
  function applyState(next) {
    ...
    if(!initialized){overview=state.view?.overview??!state.activeAgentId;for(const [key,value]of Object.entries(state.drafts||{}))drafts.set(key,value);initialized=true;if(state.recoveryNotice)toast(state.recoveryNotice,true);}
    render();
  }
  function render() {
    ...
    $('#agent-list').innerHTML=groups.filter(g=>g[1].length).map(([name,agents])=>`<div class="sidebar-section-label">${name}<span>${agents.length}</span></div>${agents.map(a=>`<button class="agent-nav ${!overview&&a.id===state.activeAgentId?'selected':''}" data-action="select" data-id="${esc(a.id)}" title="${esc(a.name+' / '+location(a)+' / '+status(a))}">${badge(a)}<span class="agent-nav-text"><strong>${esc(a.name)}</strong><small>${esc(location(a))}</small></span>${dot(a)}</button>`).join('')}`).join('')||'<div class="sidebar-empty"><span class="connection-dots">&#183; &#183; &#183;</span>Your agents will<br>feel at home here.</div>';
    const a=selected();
  ...
  function switcher(){
    ...
    const update=()=>{const q=$('#switcher-search').value.toLowerCase();$('#switcher-list').innerHTML=state.agents.filter(a=>`${a.name} ${labels[a.provider]} ${location(a)}`.toLowerCase().includes(q)).map(a=>`<button class="switcher-row" data-action="switch-select" data-id="${esc(a.id)}">${badge(a)}<span><strong>${esc(a.name)}</strong><small>${esc(location(a))}</small></span>${dot(a)}</button>`).join('')||'<p class="field-help">No matching agents. Add a connection to get started.</p>';};
  ...
  function activateTerminal(id){
    ...
    $('#terminal-title').textContent=terminalViews.get(id)?.title||`Terminal / ${selected()?.name||'no session'}`;
    $('#terminal-tabs').innerHTML=[...terminalViews.values()].map(v=>`<button class="${v.id===id?'selected':''}" data-action="terminal-tab" data-id="${esc(v.id)}">${v.exited?'&#9633;':'&#9679;'} ${esc(v.title)}</button>`).join('');
    ...
    const view=terminalViews.get(id);if(view){requestAnimationFrame(()=>{view.fit.fit();view.term.focus();});}
  }
  async function openTerminal({agentId=selected()?.id,hostId,mode='shell',local=false,terminalId,restoring=false}={}){
    ...
    const result=terminalId?await api.terminalAttach({id:terminalId}):await api.terminalOpen({agentId:hostId||local?undefined:agentId,hostId,mode,local});
    if(!terminalViews.has(result.id)){
      ...
      const term=new window.Terminal({cursorBlink:true,fontFamily:'"Cascadia Code", "SFMono-Regular", Consolas, monospace',fontSize:13,lineHeight:1.25,scrollback:4000,allowProposedApi:false,theme:{background:'#111315',foreground:'#d9dde0',cursor:'#a7f3c6',selectionBackground:'#3c4f46'}});
      ...
      const view={id:result.id,agentId:result.agentId||a?.id||'',title:result.title||`${a?.name||h?.name||(local?'This computer':'SSH')} / ${mode}`,term,fit,element,exited:!!result.exited,lastSeq:result.seq||0};terminalViews.set(result.id,view);
      term.onData(data=>action(()=>api.terminalWrite({id:result.id,data})));
      let timer;const observer=new ResizeObserver(()=>{clearTimeout(timer);timer=setTimeout(()=>{if(element.hidden)return;fit.fit();action(()=>api.terminalResize({id:result.id,cols:term.cols,rows:term.rows}));},80);});observer.observe(element);view.observer=observer;
  ...
  function terminalEvent(event){
    ...
    if(event.type==='exit'){view.exited=true;view.term.write(`\r\n\x1b[90m[Session ended: ${event.exitCode}]\x1b[0m\r\n`);activateTerminal(currentTerminal);}
  }
  document.addEventListener('click',event=>{
    ...
    if(name==='discovered-add'){openAgentForm(window.__discovered?.[Number(index)]);return;}
    ...
    action(async()=>{
      ...
      else if(name==='terminal-end'&&currentTerminal){const view=terminalViews.get(currentTerminal);if(!await api.terminalClose({id:currentTerminal}))return;view?.observer.disconnect();view?.term.dispose();view?.element.remove();terminalViews.delete(currentTerminal);activateTerminal('');}
    });
  });
