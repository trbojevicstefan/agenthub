...
(() => {
  ...
  const providerIcon = provider => ({hermes:'<span class="mark hermes-mark"><i></i><i></i></span>',codex:'<span class="mark codex-mark"><i></i></span>',claude:'<span class="mark claude-mark"><i></i><i></i></span>',openclaw:'<span class="mark openclaw-mark"><i></i><i></i><i></i></span>',custom:'<span class="mark custom-mark"><i></i></span>'}[provider]||'<span class="mark custom-mark"><i></i></span>');
  ...
  const badge = (a, large=false) => `<span class="agent-avatar ${esc(a.provider)} ${large?'large':''}" aria-label="${esc(labels[a.provider]||'Agent')} icon"><span class="provider-icon">${a.icon?esc(a.icon):providerIcon(a.provider)}</span></span>`;
  ...
  function render() {
    ...
    $('#agent-list').innerHTML=groups.filter(g=>g[1].length).map(([name,agents])=>`<div class="sidebar-section-label">${name}<span>${agents.length}</span></div>${agents.map(a=>`<div class="agent-nav-row"><button class="agent-nav ${!overview&&a.id===state.activeAgentId?'selected':''}" data-action="select" data-id="${esc(a.id)}" title="${esc(title(a)+' / '+placeText(a)+' / '+status(a))}">${badge(a)}<span class="agent-nav-text"><strong>${esc(title(a))}</strong><small>${esc(description(a))}</small></span>${dot(a)}</button><div class="agent-row-actions"><button data-action="move-agent" data-id="${esc(a.id)}" data-direction="up" title="Move up" aria-label="Move ${esc(title(a))} up">&#8593;</button><button data-action="move-agent" data-id="${esc(a.id)}" data-direction="down" title="Move down" aria-label="Move ${esc(title(a))} down">&#8595;</button><button data-action="edit" data-id="${esc(a.id)}" title="Rename, describe or delete" aria-label="Edit ${esc(title(a))}">&#8943;</button></div></div>`).join('')}`).join('')||'<div class="sidebar-empty"><span class="connection-dots">&#183; &#183; &#183;</span>Your agents will<br>feel at home here.</div>';
    const a=selected();
  ...
  function renderOverview() {
    ...
    $('#content').innerHTML=`<div class="workspace-heading"><div><div class="eyebrow"><span class="tiny-square"></span> YOUR AGENT WORKSPACE</div><h1>One place.<br>All your agents.</h1><p>From the machine in front of you to the server across the world.<br>Connect, switch, and keep the conversation going.</p></div><div class="workspace-art" aria-hidden="true"><div class="orbit orbit-one"></div><div class="orbit orbit-two"></div><div class="art-center">a<span>h</span></div><span class="art-node node-one">${providerIcon('hermes')}</span><span class="art-node node-two">${providerIcon('codex')}</span><span class="art-node node-three">${providerIcon('claude')}</span><span class="art-node node-four">${providerIcon('openclaw')}</span><span class="orbit-signal"></span></div></div><div class="workspace-stats"><div><span class="status-dot connected"></span><strong>${connected}</strong> connected</div><div><span class="machine-icon"></span><strong>${state.hosts.length}</strong> remote machines</div><div><span class="terminal-glyph">&gt;_</span> Native terminal built in</div><div class="stats-private"><span class="lock-symbol">&#9906;</span> Private by default</div></div><div class="section-heading"><div><h2>Your agents <span>${state.agents.length}</span></h2><p>Different runtimes. One familiar workspace.</p></div><button class="text-button" data-action="add">+ Add connection</button></div><div class="agent-grid">${state.agents.map(a=>`<article class="agent-card"><div class="card-top">${badge(a,true)}<span class="status-pill ${esc(a.status)}">${dot(a)}${status(a)}</span></div><h3>${esc(title(a))}</h3><div class="agent-card-meta">${meta(a)}</div><p>${esc(description(a))}</p><div class="card-connection">${envIcon(a)}${esc(connectionLabel(a))}</div><div class="card-bottom"><button class="text-button" data-action="select" data-id="${esc(a.id)}">Open workspace <span>&#8599;</span></button><button class="icon-button" data-action="edit" data-id="${esc(a.id)}" aria-label="Edit ${esc(title(a))}" title="Edit connection">&#8943;</button></div></article>`).join('')}<button class="add-card" data-action="add"><span class="add-card-plus">+</span><strong>${state.agents.length?'Make room for another.':'Meet your first agent.'}</strong><small>Hermes, Codex, Claude, OpenClaw<br>or any ACP / compatible API agent.</small><span class="add-card-link">Add an agent &#8594;</span></button></div>${!state.agents.length?'<div class="getting-started"><span class="step-number">01</span><div><strong>Already have agents installed?</strong><p>Discover checks known install folders, CLI tools and local API ports. Review what it finds before connecting.</p></div><button class="secondary" data-action="discover">Discover this computer</button></div>':''}<div class="workspace-footnote">No account to create. No credentials to route through someone else\'s server. Just your agents, connected.</div>`;
    renderKey='overview';
  }
