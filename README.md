...
`npm test` runs offline protocol, security, persistence and actual loopback-HTTP tests. `npm run smoke:restart` launches two actual Electron UI processes in sequence against one isolated session service and real native PTY. The second must recover the same service PID, live terminal, output, conversation and draft. The Windows workflow repeats this against the **installed executable**, then publishes immutable build-tagged installer assets and SHA-256 checksums. The macOS workflow builds Apple Silicon and Intel DMG/ZIP files, ad-hoc signs them, runs the same restart test against the packaged app and publishes a separate macOS preview release.
