...
class Rpc extends EventEmitter {
  // log(direction, text) receives every frame ('in'/'out'), stderr and lifecycle events for the connection log.
  constructor(child, {jsonrpc = true, onRequest = async () => { throw new Error('Unsupported agent request.'); }, log = () => {}} = {}) {
    super(); this.child = child; this.jsonrpc = jsonrpc; this.onRequest = onRequest; this.log = log; this.sequence = 0; this.pending = new Map(); this.buffer = ''; this.closed = false; this.stderr = '';
    child.stdout.setEncoding('utf8'); child.stderr.setEncoding('utf8');
    child.stdout.on('data', chunk => this.feed(chunk));
    child.stderr.on('data', chunk => { this.stderr = (this.stderr + chunk).slice(-8000); this.log('stderr', chunk); });
  ...
  fail(error) {
    if (this.closed) return;
    this.closed = true; this.log('info', `Connection closed: ${error?.message || error}`);
